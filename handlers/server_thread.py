...
def upload_task(conn, addr, chunk_manager, thread_event):
    ...
    if hello_request is None:
        ...
        conn.close()
        return
    elif hello_request.data != format_torrent_id(torrent_id=chunk_manager.torrent_id):
        logging.info(
            f" UPLOAD_THREAD({addr[0]}:{addr[1]}): Failed to validate torrent_id \nExpected:{format_torrent_id(torrent_id=chunk_manager.torrent_id)} \nActual:{hello_request.data.decode()}"
        )
        error_message = Message(
